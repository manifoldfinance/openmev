...
```sh
go get -u 'github.com/manifoldfinance/openmev@v0.0.1-alpha.1'
```
...
func main() {
	client := openmev.NewClient(
		option.WithAPIKey("My API Key"),      // defaults to os.LookupEnv("OPENMEV_API_KEY")
...
The openmev library uses the [`omitzero`](https://tip.golang.org/doc/go1.24#encodingjsonpkgencodingjson)
semantics from the Go 1.24+ `encoding/json` release for request fields.

Required primitive fields (`int64`, `string`, etc.) feature the tag <code>\`json:"...,required"\`</code>. These
fields are always serialized, even their zero values.

Optional primitive types are wrapped in a `param.Opt[T]`. These fields can be set with the provided constructors, `openmev.String(string)`, `openmev.Int(int64)`, etc.
...
```go
p := openmev.ExampleParams{
	ID:   "id_xxx",              // required property
	Name: openmev.String("..."), // optional property

	Point: openmev.Point{
		X: 0,              // required field will serialize as 0
		Y: openmev.Int(1), // optional field will serialize as 1
		// ... omitted non-required fields will not be serialized
	},

	Origin: openmev.Origin{}, // the zero value of [Origin] is considered omitted
}
```
...
custom := param.Override[openmev.FooParams](12)
```
...
client.API.V1.Public.Auctions.GetHistory(context.TODO(), ...,
	// Override the header
...
`*openmev.Error`. This contains the `StatusCode`, `*http.Request`, and
...
if err != nil {
	...
	panic(err.Error()) // GET "/api/v1/public/auctions/history": 400 Bad Request { ... }
}
```
...
defer cancel()
client.API.V1.Public.Auctions.GetHistory(
	ctx,
	openmev.APIV1PublicAuctionGetHistoryParams{},
...
We also provide a helper `openmev.File(reader io.Reader, filename string, contentType string)`
...
response, err := client.API.V1.Public.Auctions.GetHistory(
	context.TODO(),
	openmev.APIV1PublicAuctionGetHistoryParams{},
